...
__global__ void rcclWaitForAllRanksBarrier( uint8_t* flag, kernelParam kp, int rank, int nranks, int iter){
      ...
      if (tid < nranks) {
		    if (tid == 0) {
          printf("tid = %d, nranks = %d, hipBlockIdx_x*nranks + yrank = %d \n", tid, nranks, hipBlockIdx_x*nranks + rank);
			    flag[hipBlockIdx_x*nranks + rank] = iter;
      #pragma unroll
			for (int j=0; j < NPEERS; j++) {
				__atomic_store_n(&(((int*) kp.ptrs_flag[j])[hipBlockIdx_x*nranks + rank]), iter, __ATOMIC_RELAXED);
			}
		}

		/* atomic load serializes the reads among the threads. Also, flag is uncached. */
		while(__atomic_load_n(&flag[hipBlockIdx_x*nranks + tid], __ATOMIC_RELAXED) != iter) {}
...
#include "primitives.h"
  ...
  __global__ __attribute__((noinline)) void rcclWaitForAllRanksBarrier2(ncclWorkElem *args, int rank, int numRanks, uint8_t* sendbuff, uint8_t* recvbuff ) {
    ...
    Primitives<uint32_t, FuncSum<uint8_t>, FanSymmetric<1>, 0, ProtoLL, 0> prims
      (tid, nthreads, &ring->prev, &ring->next, sendbuff, recvbuff, 0, 0, 0, 0);
