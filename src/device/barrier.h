...
#else
  __device__ __attribute__((noinline)) void runRing(ncclWorkElem *args, ncclDevComm* devComm) {
#endif
    const int tid = threadIdx.x;
    ...
    if (prevRank == root) {
      ...
      for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
        offset = gridOffset + elemOffset;
        nelem = min(chunkCount, channelCount - elemOffset);
        //printf("nelem = %d offset = %d root = %d channelCount = %d elemOffset = %d \n", nelem, offset, root, channelCount, elemOffset);
...
__global__ __forceinline__ void rcclWaitForAllRanksBarrier(struct ncclDevComm* comm, struct channelMasks channelMask, struct ncclWork* workHead, ncclWorkElem *args, ncclDevComm* devComm)
{
  copyShmemData(comm, channelMask, workHead);
  runRing(args, devComm);
}