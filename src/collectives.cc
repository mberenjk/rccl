...
ncclResult_t ncclAllToAllv_impl(const void *sendbuff, const size_t sendcounts[], const size_t sdispls[],
  void *recvbuff, const size_t recvcounts[], const size_t rdispls[],
  ncclDataType_t datatype, ncclComm_t comm, hipStream_t stream) {
    INFO(NCCL_INIT, "AllToAllv is called but it is deprecated, please use AlltoAllvinstead");
  return ncclAllToAllv_impl(sendbuff, sendcounts, sdispls, recvbuff, recvcounts, rdispls, datatype, comm, stream);
}
